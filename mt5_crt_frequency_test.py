...
# The resolver probes matching broker symbols and chooses the one with the
# most M15 history inside the requested test period.
SYMBOL_ALIASES: dict[str, list[str]] = {
    "XAUUSD": ["GOLD", "XAUUSD"],
    "NAS100": ["US100Cash", "US100", "NAS100", "USTEC", "NASDAQ", "NDX"],
...
def symbol_score(mt5_name: str, aliases: list[str]) -> int:
    """Score how closely an MT5 broker symbol name matches one of our aliases."""
...
def probe_history(symbol: str, start_day: date, end_day: date) -> dict:
    """Select a broker symbol and probe requested-period M15 history."""
    ...
    return {
        ...
        "selected_ok": True,
        "bars": int(len(rates)),
        "first_bar_ny": first_utc.astimezone(NY).isoformat(timespec="minutes"),
        "last_bar_ny": last_utc.astimezone(NY).isoformat(timespec="minutes"),
        "last_error": "",
    }
...
def resolve_symbols(
    ...
    """Choose the matching broker symbol with the best requested-period M15 coverage."""
    ...
    for logical, aliases in SYMBOL_ALIASES.items():
        name_candidates = [
            (symbol_score(name, aliases), name)
            for name in names
            if symbol_score(name, aliases) >= 700
        ]
        name_candidates.sort(key=lambda x: (x[0], -len(x[1])), reverse=True)

        probes: list[dict] = []
        for score, candidate in name_candidates[:max_candidates]:
            probe = probe_history(candidate, start_day, end_day)
            probe["logical_symbol"] = logical
            probe["name_score"] = score
            probes.append(probe)
        ...
        if probes:
            ...
            best_bars = int(best["bars"])

            for probe in probes:
                probe["chosen"] = probe["candidate"] == best_name and best_bars > 0
                diagnostics.append(probe)

            if best_bars > 0:
                mapping[logical] = best_name
...
def analyze_symbol(logical: str, mt5_symbol: str, df: pd.DataFrame, sessions: str, min_bars: int) -> pd.DataFrame:
    records: list[dict] = []

    for session_day, day_df in df.groupby("ny_date", sort=True):
        if sessions in {"both", "am"}:
            rec = evaluate_session(logical, mt5_symbol, session_day, "AM", window(day_df, 1, 5), window(day_df, 5, 9), min_bars)
            if rec is not None:
                records.append(rec)

        if sessions in {"both", "pm"}:
            rec = evaluate_session(logical, mt5_symbol, session_day, "PM", window(day_df, 13, 17), window(day_df, 17, 21), min_bars)
            if rec is not None:
                records.append(rec)
...
def build_summary(details: pd.DataFrame, mapping: dict[str, str], resolver_diag: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict] = []

    for logical in SYMBOL_ALIASES:
        mapped = mapping.get(logical)
        chosen = resolver_diag[(resolver_diag["logical_symbol"] == logical) & (resolver_diag["chosen"] == True)] if not resolver_diag.empty else pd.DataFrame()
        history_bars = int(chosen.iloc[0]["bars"]) if not chosen.empty else 0
        first_bar = chosen.iloc[0]["first_bar_ny"] if not chosen.empty else None
        last_bar = chosen.iloc[0]["last_bar_ny"] if not chosen.empty else None

        if mapped is None:
            rows.append({
                "Symbol": logical,
                "MT5 Symbol": "NO USABLE HISTORY",
                "History M15 Bars": 0,
                "First Bar NY": None,
                "Last Bar NY": None,
                "Eligible Scans": 0,
        ...
        rows.append({
            "Symbol": logical,
            "MT5 Symbol": mapped,
            "History M15 Bars": history_bars,
            "First Bar NY": first_bar,
            "Last Bar NY": last_bar,
            "Eligible Scans": eligible,
            "Valid CRT": valid_count,
            "Valid %": round((valid_count / eligible * 100.0), 2) if eligible else 0.0,
            "AM Eligible": int((part["session"] == "AM").sum()) if eligible else 0,
            "PM Eligible": int((part["session"] == "PM").sum()) if eligible else 0,
    ...
    summary = summary.sort_values(["Valid %", "Valid CRT", "Eligible Scans"], ascending=[False, False, False]).reset_index(drop=True)
    summary.insert(0, "Rank", range(1, len(summary) + 1))
    return summary


def print_mapping(mapping: dict[str, str], missing: list[str], resolver_diag: pd.DataFrame) -> None:
    print("\nMT5 SYMBOL RESOLVER - HISTORY-AWARE")
    print("-" * 82)
    print(f"{'Logical':<10} {'Selected MT5 symbol':<22} {'M15 bars':>10} {'Candidates':>11}")
    print("-" * 82)

    for logical in SYMBOL_ALIASES:
        subset = resolver_diag[resolver_diag["logical_symbol"] == logical]
        chosen = subset[subset["chosen"] == True] if not subset.empty else pd.DataFrame()
        if not chosen.empty:
            row = chosen.iloc[0]
            print(f"{logical:<10} {str(row['candidate']):<22} {int(row['bars']):>10,} {len(subset):>11}")
        else:
            print(f"{logical:<10} {'NO USABLE HISTORY':<22} {0:>10,} {len(subset):>11}")
            if not subset.empty:
                alternatives = subset.sort_values(["bars", "name_score"], ascending=[False, False]).head(3)
                for _, alt in alternatives.iterrows():
                    print(f"{'':<10}   candidate: {str(alt['candidate']):<18} bars={int(alt['bars']):>6,} name_score={int(alt['name_score'])}")

    if missing:
        print("\nSymbols with no usable requested-period history:", ", ".join(missing))


def main() -> int:
    parser = argparse.ArgumentParser(description="Rank the 10 CRT Scanner V1 markets by historical valid 4H CRT frequency using MT5 M15 data.")
    ...
    parser.add_argument("--max-candidates", type=int, default=12, help="Maximum matching broker symbols to history-probe per market (default: 12)")
    ...
    if not 1 <= args.max_candidates <= 50:
        parser.error("--max-candidates must be between 1 and 50")
    ...
    print("Resolver: choose the matching MT5 symbol with the MOST requested-period M15 history.\n")
    ...
    try:
        ...
        print("\nDOWNLOADING / ANALYZING SELECTED M15 DATA")
        print("-" * 72)

        for logical in SYMBOL_ALIASES:
            mt5_symbol = mapping.get(logical)
            if not mt5_symbol:
                print(f"{logical:<10} skipped - no broker symbol with usable history")
                continue

            df = load_m15(mt5_symbol, args.start, args.end)
            if df.empty:
                print(f"{logical:<10} {mt5_symbol:<18} 0 bars - history disappeared after resolver probe")
                continue
            ...
            am_eligible = int((result["session"] == "AM").sum()) if not result.empty else 0
            pm_eligible = int((result["session"] == "PM").sum()) if not result.empty else 0
            print(f"{logical:<10} {mt5_symbol:<18} {len(df):>8,} M15 bars | {len(result):>4} eligible (AM {am_eligible:>3} / PM {pm_eligible:>3})")

        details = pd.concat(all_details, ignore_index=True) if all_details else pd.DataFrame()
        summary = build_summary(details, mapping, resolver_diag)

        print("\nCRT FREQUENCY RANKING")
        print("=" * 118)
        display_cols = ["Rank", "Symbol", "MT5 Symbol", "History M15 Bars", "Eligible Scans", "Valid CRT", "Valid %", "AM Eligible", "PM Eligible", "AM Valid", "PM Valid"]
        ...
        resolver_diag.sort_values(["logical_symbol", "bars", "name_score"], ascending=[True, False, False]).to_csv(resolver_csv, index=False)

        xlsx_path = out_dir / "crt_frequency_ranking.xlsx"
        try:
            with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
                summary.to_excel(writer, sheet_name="Ranking", index=False)
                resolver_diag.sort_values(["logical_symbol", "bars", "name_score"], ascending=[True, False, False]).to_excel(writer, sheet_name="Symbol Resolver", index=False)
        ...
        zero_history = summary[summary["History M15 Bars"] == 0]["Symbol"].tolist()
        if zero_history:
            print("\nDATA WARNING:")
            print("  No usable requested-period history for:", ", ".join(zero_history))
            print("  Do NOT treat this as a clean 10-symbol comparison yet.")
        else:
            print("\nHistory resolver passed: all 10 logical markets have requested-period M15 data.")

        no_pm = summary[(summary["AM Eligible"] > 0) & (summary["PM Eligible"] == 0)]["Symbol"].tolist()
        if args.sessions == "both" and no_pm:
            print("\nSESSION COVERAGE WARNING:")
            print("  Strict min-bars removes all PM scans for:", ", ".join(no_pm))
            print("  After history coverage is fixed, rerun with --min-bars 12")
            print("  to measure session-break sensitivity before any C3/profitability test.")

        print("\nNOTE: This test ranks CRT occurrence frequency only.")
        print("Profitability/C3 outcome testing should begin only after comparable coverage is confirmed.")
        return 0
    finally:
        mt5.shutdown()
