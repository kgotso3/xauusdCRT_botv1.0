...
def main() -> None:
    ...
    if args.start:
        times = pd.to_datetime(dataset["signal_time_utc"], utc=True, errors="coerce")
        start = pd.Timestamp(args.start)
        start = start.tz_localize("UTC") if start.tzinfo is None else start.tz_convert("UTC")
        dataset = dataset.loc[times >= start].copy()
