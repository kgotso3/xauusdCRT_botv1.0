...
def _preprocessor() -> ColumnTransformer:
    ...
    return ColumnTransformer([
        ("num", numeric, V3_NUMERIC_FEATURES),
        ("cat", categorical, V3_CATEGORICAL_FEATURES),
...
def _models(random_state: int) -> dict[str, object]:
    return {
        ...
        "extra_trees": ExtraTreesClassifier(
            n_estimators=600, max_depth=8, min_samples_leaf=8,
            class_weight="balanced", random_state=random_state, n_jobs=-1,
        ),
        "gradient_boosting": GradientBoostingClassifier(
            n_estimators=180, learning_rate=0.03, max_depth=2,
            min_samples_leaf=12, subsample=0.85, random_state=random_state,
        ),
    }
...
def run_v3_walkforward(
    dataset: pd.DataFrame,
    output_dir: str | Path = "data/research/ml_v3",
    model_dir: str | Path = "data/models/v3",
    config: V3WalkForwardConfig | None = None,
    ...
    for target_name, target_col in V12_TARGETS.items():
        ...
        for fold, train_slice, val_slice in iter_walkforward_slices(len(target_df), cfg):
            train = target_df.iloc[train_slice]
            val = target_df.iloc[val_slice]
            if len(train) < cfg.train_size or len(val) < cfg.validation_size:
                continue
        ...
        if target_fold_rows:
            ...
            summary_rows.append({
                ...
                "mean_selected_auc": float(selected["roc_auc"].mean()),
                "median_selected_auc": float(selected["roc_auc"].median()),
                "mean_selected_brier": float(selected["brier"].mean()),
                "positive_auc_folds": int((selected["roc_auc"] > 0.5).sum()),
                "best_model_mode": selected["selected_model"].mode().iloc[0],
            })
